...
def extract_face_features(landmarks) -> np.ndarray:
    ...
    # Use comprehensive facial landmarks for better recognition
    key_points = [
        1, 33, 133, 145, 263, 362, 374, 61, 291, 0, 17, 199, 94, 282
    ]

    features = []
    nose_tip = points[1]

    # Calculate relative positions to nose tip
        ...
        try:
            with open(database_file, 'r') as f:
                ...
                for name, encodings in data.items():
                    known_faces[name] = [np.array(enc) for enc in encodings]
                return known_faces
